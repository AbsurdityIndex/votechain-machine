...
func main() {
	configPath := flag.String("config", "configs/ingest.yaml", "path to ingest service config")
	flag.Parse()
	...
	application, err := app.BuildIngest(ctx, cfg, logger)
	if err != nil {
	...
	go func() {
		logger.Info("votechain ingest listening", slog.String("addr", cfg.Server.Listen))
