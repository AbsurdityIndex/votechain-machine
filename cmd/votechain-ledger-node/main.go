...
func main() {
	configPath := flag.String("config", "configs/ledger-federal.yaml", "path to ledger node config")
	flag.Parse()

	cfg, err := config.LoadLedgerNode(*configPath)
	if err != nil {
	...
	application, err := app.BuildLedgerNode(context.Background(), cfg, logger)
	if err != nil {
	...
	go func() {
		logger.Info("ledger node listening", slog.String("addr", cfg.Server.Listen), slog.String("role", cfg.Node.Role))
